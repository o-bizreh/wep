...
export class PostgresCredentialIssuer {
  async issue(
    ...
    const client = new PgClient({ connectionString: masterConnUrl.value });
    try {
      await client.connect();
      // Identifier sanitisation happened in generateUsername(); double-check.
      if (!/^[a-z0-9_]{20,32}$/.test(username)) {
      ...
      // CREATE USER doesn't accept parameters — must hand-quote. Both are sanitised above.
...
function generateUsername(): string {
  // 24 chars: prefix + 20-char hex suffix (matches the regex check above).
  return `wep_${randomBytes(10).toString('hex')}`;
}
